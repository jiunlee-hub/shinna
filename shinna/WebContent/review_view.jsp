...
<!-- 로그인 탭 -->
	...
	<div class="container mt-4 pt-4 clearfix">
		<div class="container pt-3 mt-5 pb-5 mb-4 conta2">
			<div class="h3" style="color: #B22222;">문의글보기</div>
			<hr style="height: 2.5px;" color="black" />
			<table class="table table-striped table-bordered text-center h5">
				<tr>
					<td style="width: 30%;">제목</td>
					<td colspan="2">
						<%=article.getSubject().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll("\n", "<br/>") %>
					</td>
				</tr>
				<tr>
					<td>작성자</td>
			...
			<a href="reviewList.do?page=<%=nowPage %>" class="btn btn-success float-left">목록</a>
			<%
				if(id == null) {
			%>
				<a href="reviewDeletePro.do?num=<%=article.getNum() %>&page=<%=nowPage %>" class="disabled btn btn-danger float-left ml-2" 
				onclick="return confirm('삭제를 진행하시겠습니까?')">삭제</a>
				<a href="reviewReplyForm.do?num=<%=article.getNum() %>&page=<%=nowPage %>" class="disabled btn btn-warning float-right ml-2">답변</a>
				<a href="reviewModifyForm.do?num=<%=article.getNum() %>&page=<%=nowPage %>" class="disabled btn btn-warning float-right ml-2">수정</a>
			<%
				} else {
			%>
				<a href="reviewDeletePro.do?num=<%=article.getNum()%>&ref=<%=article.getRef()%>&page=<%=nowPage %>" class="btn btn-danger float-left ml-2" 
				onclick="return confirm('삭제를 진행하시겠습니까?')">삭제</a>
				<a href="reviewReplyForm.do?num=<%=article.getNum() %>&page=<%=nowPage %>" class="btn btn-warning float-right ml-2">답변</a>
				<a href="reviewModifyForm.do?num=<%=article.getNum() %>&page=<%=nowPage %>" class="btn btn-warning float-right ml-2">수정</a>
			<%
				}
			%>
		</div>
