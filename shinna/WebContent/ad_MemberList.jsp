...
<% 
	...
	Connection con=null;
		...
		try{
			...
			pstmt=con.prepareStatement("select * from member order by country asc, name asc");
...
<div class="list mx-auto " style="width:1300px;">
	<span class="text-center" style="font-size:2em;  ">회원관리</span>
	<div>
			<div class="input-group mb-3 mt-3 " style="width:400px; float:right;">
				<select   class="form-control text-center col-3" >
					<option >아이디</option>
					<option > 이&nbsp;&nbsp;&nbsp;름</option>
				</select>
				  ...
				  <div class="input-group-append">
			    		<button type="submit" class="input-group-text btn btn-outline-secondary" id="basic-addon2">Search</button>
				  </div>
			</div>  
	</div>
	...
	<table class="table table-striped mt-4 " style="width:1300px">
	  ...
	  <tbody style="font-size:14px;">
	  	<%while(rs.next()){ %>
			<tr >
				<td class="mt-1"><%=rs.getString("country") %></td>
				<td><%=rs.getString("F_name") %></td>
				<td><%=rs.getString("L_name") %></td>
				<td><%=rs.getString("name") %></td>
				<td><%=rs.getString("id") %></td>
				<td><%=rs.getString("pw") %></td>
				<td class="text-center"><%=rs.getInt("yy") %>-<%=rs.getInt("mm") %>-<%=rs.getInt("dd") %></td>
				<td >(<%=rs.getString("tel_1") %> )<br><%=rs.getString("tel_2") %></td>
				<td><%=rs.getString("e_mail") %>@<%=rs.getString("e_mail2") %></td>
				<td class="text-center"><%=rs.getString("gender") %></td>
				<td class="text-center"><a href="ad_MemberDelete.do?id=<%=rs.getString("id") %>" 
